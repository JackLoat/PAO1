...
void MainWindow::on_sensorClicked(QListWidgetItem* item){
    ...
    ui->box_sensore->tipo->setText("Tipo: "+sensor->getTipo());
    ui->box_sensore->dataC->setText("Data C: "+sensor->getDataC().toString("dd/MM/yyyy"));
    ui->box_sensore->stanza->setText("Stanza: "+sensor->getStanza());
    ui->box_sensore->collezione->setText("Collezione: "+sensor->getCollezione()->getNome());
    ...
    ui->box_sensore->setProperty("PuntatoreLista", QVariant::fromValue(static_cast<void*>(item)));

    connect(ui->box_sensore->rimuovi, &QListWidget::itemClicked, this, &MainWindow::)
}
