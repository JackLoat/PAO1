...
visitor_dati::visitor_dati(int n, double  m, double M):visitor_dati(n){
    min = m;
    max = M;
}

visitor_dati::visitor_dati(int n, double m, double M, double s):visitor_dati(n,m,M){
    soglia = s;
}
...
void visitor_dati::visit(sensore_fumo* fumo){
    //non ci sono parametri da settare sul sensore, basta passare periodo_dati alla funzione
...
void visitor_dati::visit(sensore_aria* aria){
    //setta campi del sensore: min, max, soglia
    aria->setMin(min);
    aria->setMax(max);
...
//soluzione poco elegante ma veloce da implementare; serve solo a mantenere più pulita la funzione seguente.
